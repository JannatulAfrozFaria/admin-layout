...
const OrderSummary = () => {
    const OrderStats = [
        ...
        { id: 2, title: 'Pending', number: 125, percentage: 20, value: 50, color: '#41a3ff' },
    ...
    const chartData = {
        ...
        datasets: [
            {
                data: OrderStats.map(item => item.number),
                backgroundColor: OrderStats.map(item => item.color),
                hoverBackgroundColor: OrderStats.map(item => item.color),
            },
        ],
    };
                ...
                <div className="grid grid-cols-1 gap-1">
                    {OrderStats.map(item => (
                        <div key={item.id}>
                            ...
                            <progress className={`progress w-full`} value={item.value} max="100"></progress>
                        </div>
                    ))}
                </div>
            </div>
